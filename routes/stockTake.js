...
async function addTrayMany(body, dbo) {
  ...
  for (let i = 0; i < body.length; i++) {
    ...
    if (
      !(
        tray.hasOwnProperty("zone") &&
        tray.hasOwnProperty("bay") &&
        tray.hasOwnProperty("tray") &&
        tray.hasOwnProperty("contents") &&
        tray.hasOwnProperty("expiry") &&
        tray.hasOwnProperty("weight") &&
        tray.hasOwnProperty("xPos") &&
        tray.hasOwnProperty("yPos")
      )
    ) {
    ...
    if (
      !(
        typeof tray["zone"] === "string" &&
        typeof tray["bay"] === "string" &&
        typeof tray["tray"] === "string" &&
        typeof tray["contents"] === "string"
      )
    ) {
    ...
    if (tray["xPos"] < 0 || tray["yPos"] < 0) {
      console.log(
        "Position must be within the valid range! (Positive Integer)"
      );
      return "FAIL";
    }
  }
...
async function getAllCategory(body, dbo) {
  ...
  const trays = await dbo
    .collection("food")
    .find({ contents: body["contents"].contents })
    .toArray();
  return trays;
}
...
async function getNextNExpiring(body, dbo) {
  ...
  for (let count = 0; count < trays.length; count++) {
    if (trays[count]["expiry"].length == 4) {
      ...
      let x = new Date(
        parseInt(expiryArray[1]),
        parseInt(expiryArray[0]) - 1,
        1,
        0,
        0,
        0
...
async function addZone(zone, dbo) {
  if (
    !(
      zone.hasOwnProperty("zone") &&
      zone.hasOwnProperty("height") &&
      zone.hasOwnProperty("width")
    )
  ) {
    console.log("Malformed request!");
    return "FAIL";
  ...
  try {
    ...
    await dbo
      .collection("zones")
      .updateOne(pos, { $push: { bays: newValues } });
  } catch (ex) {
    console.log(ex);
    return "FAIL";
  }
...
async function editZone(zone, dbo) {
  ...
  if (
    !(
      newValues.hasOwnProperty("width") ||
      newValues.hasOwnProperty("height") ||
      newValues.hasOwnProperty("zone")
    )
  ) {
    return "SUCCESS";
  }
...
async function addBay(bay, dbo) {
  if (
    !(
      bay.hasOwnProperty("bay") &&
      bay.hasOwnProperty("zone") &&
      bay.hasOwnProperty("xVal") &&
      bay.hasOwnProperty("yVal") &&
      bay.hasOwnProperty("xSize") &&
      bay.hasOwnProperty("ySize")
    )
  ) {
    console.log("Malformed request!");
    return "FAIL";
  ...
  if (
    !(
      Number.isInteger(bay["xVal"]) &&
      Number.isInteger(bay["yVal"]) &&
      Number.isInteger(bay["xSize"]) &&
      Number.isInteger(bay["ySize"])
    )
  ) {
  ...
  if (
    ...
    console.log(
      "Position and size must be within the valid range! (Positive Integer)"
    );
    return "FAIL";
  }
...
async function removeBay(bay, dbo) {
  ...
  try {
    ...
    await dbo
      .collection("zones")
      .updateOne(pos, { $pull: { bays: bay["bay"] } });
  } catch (ex) {
    console.log(ex);
    return "FAIL";
  }
...
async function addTray(tray, dbo) {
  ...
  try {
    let res = await dbo
      .collection("food")
      .updateOne(pos, { $set: tray }, { upsert: true });
...
async function switchTray(body, dbo) {
  ...
  if (
    !(
      first.hasOwnProperty("zone") &&
      first.hasOwnProperty("bay") &&
      first.hasOwnProperty("tray")
    )
  ) {
    console.log("Malformed request");
    return "FAIL";
  ...
  if (
    !(
      second.hasOwnProperty("zone") &&
      second.hasOwnProperty("bay") &&
      second.hasOwnProperty("tray")
    )
  ) {
    console.log("Malformed request");
    return "FAIL";
  ...
  if (
    !(
      ...
      typeof first["bay"] === "string" &&
      typeof first["tray"] === "string"
    )
  ) {
  ...
  if (
    !(
      typeof second["zone"] === "string" &&
      typeof second["bay"] === "string" &&
      typeof second["tray"] === "string"
    )
  ) {
...
async function moveTray(body, dbo) {
  ...
  if (
    !(
      posStart.hasOwnProperty("zone") &&
      posStart.hasOwnProperty("bay") &&
      posStart.hasOwnProperty("tray")
    )
  ) {
    console.log("Malformed request");
    return "FAIL";
  ...
  if (
    !(
      posTarget.hasOwnProperty("zone") &&
      posTarget.hasOwnProperty("bay") &&
      posTarget.hasOwnProperty("tray")
    )
  ) {
    console.log("Malformed request");
    return "FAIL";
  ...
  if (
    !(
      typeof posStart["zone"] === "string" &&
      typeof posStart["bay"] === "string" &&
      typeof posStart["tray"] === "string"
    )
  ) {
  ...
  if (
    !(
      typeof posTarget["zone"] === "string" &&
      typeof posTarget["bay"] === "string" &&
      typeof posTarget["tray"] === "string"
    )
  ) {
  ...
  try {
    var occupied = true;
    occupied =
      (await dbo.collection("food").find(posTarget).limit(1).length) > 0;
    ...
    await dbo
      .collection("food")
      .updateOne(posStart, { $set: posTarget }, function (err, res) {
        if (err) throw err;
...
async function mongoUpdate(body, method) {
  ...
  const URL =
    ...
    "@cluster0-oxrvp.mongodb.net/test?retryWrites=true&w=majority";
